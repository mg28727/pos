...
public class SystemInit extends JFrame
{
	 ...
	 * @throws IOException 
	 */
	SystemInit() throws IOException
	{File f = new File("Printers.dat");
	if(!f.exists())
	{PrinterClass app=new PrinterClass();
	app.createFile();
	}
		setUndecorated(true);
	...
	public static void setPrinterScreen() throws IOException{
		systemPanel.removeAll();
		systemPanel.add(new PrinterGUI());
		Tools.update(systemPanel);
	}
	
	public static void setPrinterSettings(String printerName) throws IOException{
		systemPanel.removeAll();
		systemPanel.add(new PrinterSettingsGUI(printerName));
		Tools.update(systemPanel);
	}
}
